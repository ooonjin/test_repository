...
<?php
    include "./sideR.php";      //contents_main2_1L_low.php를 기본메인으로 설정, 고정 레이아웃 sideR.php만 추가
?>
...
<div style="float:left;">
    <form method="POST">    <!-- POST방식으로 라디오버튼값 전달 -->
        <!-- 용량 : <input type="radio" name="1L_down" value=' ' onclick="document.location.href='MAIN.php'"/>1L 이하
            ...
            용량 : <input type="radio" name="1L" value="1" onclick="document.location.href='MAIN.php'" checked/>1L 이하
            <input type="radio" name="1L" value="2" onclick="document.location.href='contents_main2_1L_over.php'"/>1L 이상<br/>
    </form>
</div>
...
<?php
    ...
    $sql = "SELECT FACTORY_NAME, PRODUCT_NAME, CAPACITY, PRICE, IMAGE, PRODUCT_NUMBER       
            ...
            ORDER BY PRICE ASC";                                    //변수 sql에, SQL문에 이미지 추가
...
<?php //echo "<a href =\"DETAILS.php?pn=".$row["PRODUCT_NAME"]."\">".$row["PRODUCT_NAME"]."</a>";?>
<?php //echo "<a href =\"DETAILS.php?pr=".$row["PRICE"]."\">".$row["PRICE"]."원"."</a>";?>
<?php //echo "<td>"."<a href =\"view.php?no=".$row["num"]."\">".$row["title"]."</a>"."</td>";?>